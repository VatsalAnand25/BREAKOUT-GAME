...
# Block properties
# Adjust block properties to fill the screen exactly
block_spacing = 5  # Space between blocks
rows = 5
block_width = 100 # Recalculate block width dynamically
cols = (screen_width + block_spacing) // (block_width + block_spacing)
...
font = pygame.font.Font(None, 36)
  ...
  # Ensure one extra column fits
for i in range(rows):
    for j in range(cols):
        blocks.append(pygame.Rect(j * (block_width + 5), i * (block_height + 5), block_width, block_height))
...
ball = pygame.Rect(screen_width // 2 - ball_radius // 2, screen_height - paddle_height - ball_radius - 20, ball_radius * 2, ball_radius * 2)
